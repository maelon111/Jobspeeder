...
import {
  Document, Page, View, Text, StyleSheet, pdf, Font,
...
function MinimalPDF({ data, color }: { data: ResumeData; color: string }) {
  const p = data.personal
  const s = StyleSheet.create({
...
function DarkPDF({ data, color }: { data: ResumeData; color: string }) {
  const p = data.personal
  ...
  const s = StyleSheet.create({
    ...
    firstName: { fontSize: 16, fontFamily: 'Helvetica', color: '#ffffff', letterSpacing: 2, textTransform: 'uppercase' },
    lastName: { fontSize: 36, fontFamily: 'Helvetica-Bold', color, letterSpacing: 1, textTransform: 'uppercase', lineHeight: 1 },
    title: { fontSize: 10, textTransform: 'uppercase', color: '#aaaaaa', marginTop: 5 },
    summary: { fontSize: 9, color: '#727171', lineHeight: 1.7, marginTop: 8, maxWidth: 380 },
    cols: { flexDirection: 'row', gap: 28, marginTop: 20 },
    left: { flex: 1 },
    right: { flex: 1 },
    sectionTitle: { fontSize: 10, fontFamily: 'Helvetica-Bold', color, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 10 },
    ...
    contactLabel: { fontSize: 9, fontFamily: 'Helvetica-Bold', color, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 },
...
export async function POST(req: NextRequest) {
  try {
    const { data, template, primaryColor } = await req.json() as {
      data: ResumeData
    ...
    if (template === 'minimal') {
      element = React.createElement(MinimalPDF, { data, color })
    } else if (template === 'creative') {
      element = React.createElement(CreativePDF, { data, color })
    } else if (template === 'dark') {
      element = React.createElement(DarkPDF, { data, color })
    } else {
      element = React.createElement(ModernPDF, { data, color })
    }
